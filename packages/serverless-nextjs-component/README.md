...
| region        | `string`          | `us-east-1`              | The region where s3 bucket resides (and the lambdas/cloudfront distribution reports as the main region although they are deployed to every edge)                                                                                                                     |
...
You can configure the `region` for resources like `s3 bucket` via the region input configuration.
